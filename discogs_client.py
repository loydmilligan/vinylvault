...
class DiscogsRateLimiter:
    ...
    def __init__(self, max_requests: int = 55, window: int = 60):
        """Initialize rate limiter with conservative defaults."""
        self.max_requests = max_requests  # Slightly under the 60/min limit
        self.window = window
        self.requests = []
...
class DiscogsClient:
    ...
    def _extract_release_data(self, release) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Extract images
            images = []
            if hasattr(basic_info, 'images') and basic_info.images:
    ...
    def _process_collection_item(self, item_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            if item_data['images']:
                ...
                if primary_image:
                    cover_url = primary_image.get('uri600', primary_image.get('uri'))
